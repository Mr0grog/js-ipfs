...
- The listener (`STOP`) knows how to process relay requests that come from the `HOP` node
...
The bundled example is a simple chat app that uses another cool ipfs feature - [pubsub](https://github.com/libp2p/js-libp2p-floodsub). Lets open up a browser and paste the above url into the address bar. We should see something similar to the following image:
